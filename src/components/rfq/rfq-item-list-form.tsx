...
export function createDefaultRfqItems(): RfqItem[] {
  return [{ id: "rfq-item-0", group: "", description: "", quantity: 1, unit: "", price: 0 }];
}

export function calculateRfqItemsTotal(items: RfqItem[]) {
  return items.reduce((sum, item) => sum + item.quantity * item.price, 0);
}

export function RfqItemListForm({
  items,
  onChange,
}: {
  items: RfqItem[];
  ...
  function updateItem<K extends keyof RfqItem>(
    id: string,
    field: K,
    value: RfqItem[K]
  ) {
  ...
  const total = calculateRfqItemsTotal(items);
  const hasValidItem = items.some((item) => item.description.trim());

  return (
    <Card>
      <CardHeader>
        <CardTitle>Item Diminta Penawaran</CardTitle>
        <CardDescription>
          Barang/jasa yang dimintakan penawaran harga. Harga estimasi opsional
          (diisi vendor saat menjawab).
        </CardDescription>
      </CardHeader>
          ...
          >
            ...
            <div className="col-span-12 grid gap-1.5 sm:col-span-5">
              ...
              <Input
                ...
                placeholder="Nama barang/jasa yang diminta"
