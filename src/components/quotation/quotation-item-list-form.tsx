...
export function createDefaultQuotationItems(): QuotationItem[] {
  return [
    { id: "quo-item-0", group: "", description: "", spec: "", quantity: 1, unit: "", price: 0 },
  ];
}

export function calculateQuotationItemsTotal(items: QuotationItem[]) {
  return items.reduce((sum, item) => sum + item.quantity * item.price, 0);
}

export function QuotationItemListForm({
  items,
  onChange,
}: {
  items: QuotationItem[];
  ...
  function addItem() {
    const id = `quo-item-${nextId}`;
    setNextId((n) => n + 1);
    const lastGroup = items[items.length - 1]?.group ?? "";
    onChange([
      ...items,
      { id, group: lastGroup, description: "", spec: "", quantity: 1, unit: "", price: 0 },
    ]);
  }
  ...
  const total = calculateQuotationItemsTotal(items);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Rincian barang/jasa yang ditawarkan beserta spesifikasi & harganya.
        </CardDescription>
      </CardHeader>
          ...
          >
            ...
            <div className="col-span-12 grid gap-1.5 sm:col-span-3">
              ...
              <Input
                ...
                placeholder="mis. 10cm, food grade"
                value={item.spec}
